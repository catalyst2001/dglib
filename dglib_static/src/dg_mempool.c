...
int mempool_init(dg_mempool_t* pdst, uint32_t poolsize, uint32_t blocksize)
{
	...
	/* add all free pool indices */
	for (uint32_t i = 0; i < pdst->poolsize; i++) {
		if (!mpmc_queue_add_back(&pdst->fqueue, &i)) {
			DG_ERROR("mempool_init(): mpmc_queue_add_back() returned false!");
			return 0;
		}
	}
...
{
	return (block >= (void*)psrc->pdata && block < (void*)(psrc->pdata + psrc->poolsize * psrc->blocksize));
}
...
int mempool_free(dg_mempool_t* psrc, void* block)
{
	...
	if (!mempool_is_valid_addr(psrc, block)) {
		DG_ERROR("mempool_free(): block ptr is out of pool bounds");
		return 0;
	}
...
{
	if (!psrc || !psrc->poolsize)
		return 0;

	/* If queue not yet allocated (before init) */
	if (!psrc->fqueue.capacity)
		return 0;

	size_t enq = dg_atomic_load(&psrc->fqueue.enqueue_pos);
	size_t deq = dg_atomic_load(&psrc->fqueue.dequeue_pos);
	size_t qcount = (enq - deq);
	if (qcount > psrc->fqueue.capacity)
		qcount = psrc->fqueue.capacity; /* safety clamp */

	return (uint32_t)qcount;
}
...
uint32_t mempool_get_num_allocated(const dg_mempool_t* psrc)
{
	...
	uint32_t free_cnt = mempool_get_num_free(psrc);
	if (free_cnt > psrc->poolsize)
		return 0; /* inconsistent, fallback */

	return psrc->poolsize - free_cnt;
}
