...
#define calloc(size, count) (gpmmdt->mem_alloc_debug(NULL, size * count, DGMM_CLEAR, __FILE__, __LINE__))
...
#define calloc(size, count) (gpmmdt->mem_alloc(NULL, size*count, DGMM_CLEAR))
#define realloc(ptr, size) (gpmmdt->mem_alloc_debug(ptr, size, DGMM_COPY))
