...
*/
DG_API bool fs_get_path_by_id(dg_path_t* pdst, pathid_t pathid);
...
*/
DG_API int fs_open_file(file_t* pdst,
	pathid_t pathid,
	const wchar_t* ppath,
	uint32_t mode,
	uint32_t accs);
...
*/
DG_API int fs_open_buffer(file_t* pdst,
	void* pbuffer,
	size_t size, 
	uint32_t mode,
	uint32_t accs);
...
DG_API int fs_finfo(fs_finfo_t *pdst, file_t hfile);
...
* @return 2 - invalid handle
*/
DG_API int fs_seek(file_t hfile, int64_t pos, int32_t mode);
...
*/
DG_API int64_t fs_tell(file_t hfile);
...
* @return nothing
*/
DG_API void fs_close(file_t hfile);
...
*/
DG_API int fs_write(file_t hfile, const void* psrc, size_t srclen, size_t *pnbyteswrite);
DG_API int fs_read(void* pdst, size_t dstlen, file_t hfile, size_t* pnbytesread);
...
DG_API int fs_write_string(file_t hfile, const char* psrc, size_t length);
DG_API const char* fs_read_string(char* pdst, size_t maxlen, size_t length, file_t hfile);

/**
* automatic byte-swapped reading/writing
*/

DG_API int fs_read_s16(int32_t* pdst, file_t hfile, int endian);
DG_API int fs_read_u16(uint32_t* pdst, file_t hfile, int endian);
DG_API int fs_read_s32(int32_t* pdst, file_t hfile, int endian);
DG_API int fs_read_u32(uint32_t* pdst, file_t hfile, int endian);
DG_API int fs_read_s64(int64_t* pdst, file_t hfile, int endian);
DG_API int fs_read_u64(uint64_t* pdst, file_t hfile, int endian);
...
DG_API int fs_write_s16(file_t hfile, int32_t src, int endian);
DG_API int fs_write_u16(file_t hfile, uint32_t src, int endian);
DG_API int fs_write_s32(file_t hfile, int32_t src, int endian);
DG_API int fs_write_u32(file_t hfile, uint32_t src, int endian);
DG_API int fs_write_s64(file_t hfile, int64_t src, int endian);
DG_API int fs_write_u64(file_t hfile, uint64_t src, int endian);
